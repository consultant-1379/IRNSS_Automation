...
cursor = db.cursor()
sql = "truncate temp_irn_diskusage_table"
cursor.execute(sql)
db.commit()
############Sacinng and polling 
                            ...
                            for delete_data in delete_results:
                                 ...
                                 if int(verif_dir_capacity) < 85:
                                    ...
                                    for line in sucess_result:
                                       ...
                                       print "File       :",line[2]
                                       sql = "SELECT COUNT,DIRECTORY,FILE_PATH,SOLUTION FROM irn_diskusage_knowledge_table where DIRECTORY='"+line[1]+"' and FILE_PATH='"+line[2]+"'"
                                       cursor.execute(sql)
                                       ...
                                       if line[1] in str(update_kedb) and line[2] in str(update_kedb):
                                          ...
                                          Count_update= "UPDATE irn_diskusage_knowledge_table SET COUNT='"+str(Count)+"' WHERE DIRECTORY='"+line[1]+"' and FILE_PATH='"+line[2]+"'"
                                          cursor.execute(Count_update)
                                          db.commit()
                                       else:
                                          cursor.execute("""INSERT INTO irn_diskusage_knowledge_table(IP_ADDRESS,COUNT,DIRECTORY,FILE_PATH,SOLUTION)VALUES (%s,%s,%s,%s,%s)""",(line[0],"1",line[1],line[2],"Delete"))
                                          cursor = db.cursor()
                                          db.commit()
